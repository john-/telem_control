...
sub init {
    ...
    $self->node->{device}{chip}
      ->mount_from_paramstr( $adapter, $self->node->{device}{params}, )->get;
    my $result = eval {

        #my $voltage = $chip->read_bus_voltage->get;
        my $ret =
          $self->node->{device}{reader}->( $self->node->{device}{chip} );
