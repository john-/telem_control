...
sub _initialize {
    ...
    my $fc = $self->{config}{fan_control}
      ;    # TODO:  maybe TL should only get config it needs
    ...
    Mojo::IOLoop->recurring(
        $fc->{rate} => sub {
            ...
            my $results =
              $self->{pg}->db->query(
'select value from sensor_history where input = \'cpu_temp\' order by reading_key desc limit 1'
              ) or $self->{log}->error("could not retrieve cpu temperature");
...
# If your number X falls between A and B, and you would like Y to fall between C and D, you can apply the following linear transform:
            ...
            my $percent = sprintf( '%d',
                ( $cpu_temp - $fc->{temp}{min} ) /
                  ( $fc->{temp}{max} - $fc->{temp}{min} ) *
                  ( $fc->{speed}{max} - $fc->{speed}{min} ) +
                  $fc->{speed}{min} );
            ...
            $self->{log}->info(
                sprintf(
'fan control: cpu_temp is %.1fF, changing fan speed to %d%%',
                    $cpu_temp, $percent
                )
            );
